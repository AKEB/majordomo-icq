<?php
  ...
  global $sortby_user;
  if (!$sortby_user) {
   $sortby_user=$session->data['icq_user_sort'];
  } else {
   if ($session->data['icq_user_sort']==$sortby_user) {
   ...
   $session->data['icq_user_sort']=$sortby_user;
  }
  if (!$sortby_user) $sortby_user="USER_ID";
  $out['SORTBY']=$sortby_user;
  
  // SEARCH RESULTS  
  $res=SQLSelect("SELECT * FROM icq_user ORDER BY ".$sortby_user);
  if ($res[0]['ID']) {  
