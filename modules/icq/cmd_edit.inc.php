...
$res = SQLSelect("select ID,NAME,ADMIN, (SELECT count(*) FROM icq_user_cmd where CMD_ID='$id' and icq_user_cmd.USER_ID=icq_user.ID) as ACCESS_USER from icq_user");
if ($res[0]) {
    $out['LIST_ACCESS'] = $res;
}
$res = SQLSelect("SELECT * from icq_user_cmd where CMD_ID='$id'");
if ($res[0]) {
    $qs = array();
    foreach ($res as $row) {
        $qs[] = $row['USER_ID'];
...
{
    SQLSelect("DELETE from icq_user_cmd where CMD_ID=" . $cmd_id);
    $users = explode(",", $users_id);
    foreach ($users as $value) {
        $recCU = array();
        $recCU['CMD_ID'] = $cmd_id;
        $recCU['USER_ID'] = $value;
        $recCU['ID'] = SQLInsert('icq_user_cmd', $recCU);
    }
}
