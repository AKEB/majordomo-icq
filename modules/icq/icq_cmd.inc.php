<?php
  ...
  global $sortby_cmd;
  if (!$sortby_cmd) {
   $sortby_cmd=$session->data['icq_sort_cmd'];
  } else {
   if ($session->data['icq_sort_cmd']==$sortby_cmd) {
   ...
   $session->data['icq_sort_cmd']=$sortby_cmd;
  }
  if (!$sortby_cmd) $sortby_cmd="TITLE";
  $out['SORTBY']=$sortby_cmd;
  
  // SEARCH RESULTS  
  $res=SQLSelect("SELECT * FROM icq_cmd ORDER BY ".$sortby_cmd);
  if ($res[0]['ID']) {   
