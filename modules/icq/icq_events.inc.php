<?php
  ...
  global $sortby_event;
  if (!$sortby_event) {
   $sortby_event=$session->data['icq_sort_event'];
  } else {
   if ($session->data['icq_sort_event']==$sortby_event) {
   ...
   $session->data['icq_sort_event']=$sortby_event;
  }
  if (!$sortby_event) $sortby_event="TITLE";
  $out['SORTBY']=$sortby_event;
  
  // SEARCH RESULTS  
  $res=SQLSelect("SELECT * FROM icq_event ORDER BY ".$sortby_event);
  if ($res[0]['ID']) {   
